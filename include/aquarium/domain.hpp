...
struct Species {
 ...
 double nominalLength{38};int eventStart{},eventEnd{};bool eventConfigured{true},annual{},oneTime{},artReady{true};
 std::string releaseGate{"Launch"},scheduleId;
 Millis durationMs{};int coinWeightBps{10000},xpWeightBps{10000},coinFactorBps{10000},xpFactorBps{10000};
};
struct Economy {
 Amount baseCoinDay{360},baseXpDay{576},minimumPrice{5};
 int coinSlopeBps{450},xpSlopeBps{1500},principalShareBps{2500},earlyRefundBps{9500};
...
struct DecorTuning {int placedLimit{64},animatedLimit{6},emitterLimit{2},particleLimit{6},launchLevelCap{40};Amount tutorialTotalXp{15};std::string tutorialItem{"CP-01"};};
struct Content {
 std::string configVersion;Economy economy;Millis hatchMs{6000};
 std::vector<TankEntitlement> tankEntitlements;std::array<Money,40> levelRewards{};
 int pearlSalesTarget{20};Amount pearlSalesReward{1};
 std::vector<TreasureOffer> treasureOffers;int referenceUtilizationNumerator{5},referenceUtilizationDenominator{6};
 std::array<int,40> referenceSlotsByLevel{};
 std::vector<Species> species;std::array<Amount,40> levels{};Json workbook;Json supplement;
 std::array<std::array<Amount,7>,5> tankCosts{};
 std::array<std::array<Amount,7>,5> tankPearlCosts{};
...
struct Motion {
 ...
 std::optional<WorldPoint> sellTarget;
 double sellSpeed{};
 double fleeRemaining{};
};
struct SellTarget {FishId fish;WorldPoint position;};
struct WaterRipple {WorldPoint position;double age{},previousAge{};static constexpr double lifetime=1.3;};
...
bool pearlSaleEligible(const Content&,const Fish&);
...
// Swimming uses the full visible tank. Saved decor spans the complete artwork
// and the view projects it with the background's crop.
constexpr double waterWidth=1088,tankHeight=635,waterHeight=tankHeight;
// Fish, plants and decorations display at 120 percent of their base size.
constexpr double tankArtScale=1.2;
...
// Drawing and selection follow bottom-anchor height, independently of size.
bool decorBehind(const Decoration&,const Decoration&);
...
struct Settings {
 bool reducedMotion{},sound{true},music{true};double volume{.65};int tankLook{};
 bool vibration{true},reducedMotionOverride{};
};
struct State {
...
// Value 33 is retired. Keep later values stable for saved command receipts.
enum class Action {Buy,Feed,Sell,Stash,Restore,Move,Revive,Remove,ReviveAll,RemoveAll,UnlockTank,ExpandTank,SwitchTank,DropFood,BuyDecor,ClaimQuest,SendGift,DailyEgg,ClaimMastery,Tutorial,SetLook,SetReducedMotion,SetSound,SetMusic,SetVolume,MoveDecor,StoreDecor,RestoreDecor,FlipDecor,PurchaseDecor,PlaceDecor,CancelDecor,ResizeDecor,Favorite=34,PurchaseEnvironment,EquipEnvironment,SetVibration,SetSystemReducedMotion};
...
class Domain {
 ...
 const std::vector<WaterRipple>& ripples()const{return ripples_;}
 ...
 // Temporary swimming destinations, cleared when the selling tool closes.
 void arrangeForSale(std::span<const SellTarget>);
 // Empty-water feedback is transient and does not issue a saved command.
 void tapWater(WorldPoint);
