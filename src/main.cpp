...
int main(int argc,char** argv){
 try{
  ...
  const auto prepareStart=SDL_GetTicksNS();view.prepareMenus();const double menuPreparationMs=double(SDL_GetTicksNS()-prepareStart)/1e6;
  ...
  session.checkpoint(wallNow());if(!o.report.empty()){
   ...
   report["menu_preparation_ms"]=menuPreparationMs;
