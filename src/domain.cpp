...
Content Content::fromJson(const Json& j){
 ...
 for(const auto& x:j.at("species")){
  ...
  s.level=x.at("level");std::string currency=x.at("currency");s.currency=currency=="pearls"?Currency::Pearls:Currency::Coins;
  check(currency=="coins"||currency=="pearls","Invalid fish currency");
  s.price=x.at("price");s.buyXp=x.at("buy_xp");s.durationMs=x.at("duration_ms");s.stageMs=s.durationMs;s.feedMs=x.at("feed_ms");
  s.releaseGate=x.at("release_gate");s.scheduleId=x.at("schedule_id");s.artReady=x.at("art_ready");s.coinWeightBps=x.at("coin_weight_bps");s.xpWeightBps=x.at("xp_weight_bps");s.coinFactorBps=x.at("coin_factor_bps");s.xpFactorBps=x.at("xp_factor_bps");
  ...
  s.eventStart=x.value("event_start",0);s.eventEnd=x.value("event_end",0);s.eventConfigured=x.value("event_configured",true);s.annual=x.value("annual",false);s.oneTime=x.value("one_time",false);
  check(s.durationMs>=1200000&&s.durationMs<=158400000,"Invalid total growth duration");
  check(s.feedMs>0&&s.feedMs<=43200000&&s.price>=0&&s.price<=limit&&s.buyXp==0&&s.level>=1&&s.level<=70,"Invalid price or meal duration");
  {
   check(!s.scheduleId.empty()&&s.scheduleId.size()<128&&s.coinWeightBps>=9000&&s.coinWeightBps<=11000&&s.xpWeightBps>=9000&&s.xpWeightBps<=11000&&s.coinFactorBps>0&&s.coinFactorBps<=15000&&s.xpFactorBps>0&&s.xpFactorBps<=15000,"Invalid production weights");
 ...
 check(c.species.size()==99,"Expected 99 fish species");
 if(j.contains("decorations")){
  ...
  for(const auto& x:dc.at("items")){
   ...
   check(!c.findDecor(d.id)&&d.level>=1&&d.level<=70&&d.price>0&&d.price<=limit&&d.buyXp>0&&d.buyXp<=limit,"Invalid decor catalog identity, price or XP");
 ...
 const auto& inputs=j.at("inputs");c.startingWallet={inputs.at("starting_coins"),inputs.at("starting_pearls")};c.startingTankCapacity=inputs.at("starting_slots");c.hatchMs=j.at("overrides").at("hatch_ms");
 check(c.hatchMs==6000&&c.startingTankCapacity==10,"Invalid starting capacity or hatch time");
 ...
 for(const auto& x:j.at("tank_entitlements")){
  ...
  check(t.tank.value>=1&&t.tank.value<=5&&t.slots>=10&&t.slots<=40&&t.slots%5==0&&(t.slots==10?t.level>=1&&t.level<=40:t.level==0)&&t.addedSlots==(t.slots==10?10:5)&&t.cost.coins>=0&&t.cost.pearls>=0,"Invalid tank entitlement");
 ...
 check(c.tankEntitlements.size()==35&&slots==200,"Invalid total capacity");
 const auto& treasure=j.at("treasure");
 c.referenceSlotsByLevel=treasure.at("reference_slots_by_level").get<std::array<int,40>>();
 check(c.referenceSlotsByLevel.front()==10&&c.referenceSlotsByLevel.back()==100&&std::is_sorted(c.referenceSlotsByLevel.begin(),c.referenceSlotsByLevel.end()),"Invalid reference capacity curve");
 ...
 c.pearlSalesTarget=j.at("pearl_earning").at("sales_target");c.pearlSalesReward=j.at("pearl_earning").at("reward");
 check(c.pearlSalesTarget>=1&&c.pearlSalesTarget<=1000&&c.pearlSalesReward>=1&&c.pearlSalesReward<=100,"Invalid pearl earning rule");
 c.starters=j.at("starter_species").get<std::vector<std::string>>();for(const auto& id:c.starters)check(c.find(id)&&c.find(id)->level==1&&c.find(id)->currency==Currency::Coins,"Invalid gifted starter fish");
 return c;
}
...
std::string errorText(Error e){switch(e){case Error::Conflict:return "The offer changed. Please review it and try again.";case Error::SaveFailure:return "The action was not saved. Please retry.";case Error::Protected:return "Unfavorite this fish before selling it.";case Error::None:return "";case Error::Unknown:return "That item is not available.";case Error::NoArt:return "Artwork unavailable. Nothing was charged.";case Error::EventClosed:return "This seasonal event is closed.";case Error::Level:return "Reach the required level first.";case Error::Claimed:return "Already claimed for this period.";case Error::Full:return "This tank is full. Sell a fish, expand or switch tanks.";case Error::Funds:return "Open the shop for more coins or pearls.";case Error::InvalidPosition:return "Place inside the aquarium.";case Error::InvalidFish:return "That fish is no longer here.";case Error::NotSellable:return "This fish has no sale rewards.";case Error::Dead:return "A dead fish cannot be stored or fed.";case Error::NotDead:return "This fish is already alive.";case Error::NotStored:return "That fish is not in inventory.";case Error::AlreadyOwned:return "You already own this tank.";case Error::PreviousTank:return "Unlock the preceding tank first.";case Error::Maximum:return "Maximum capacity reached.";case Error::NoFoodRoom:return "There is enough food in the water already.";case Error::Unavailable:return "This external service is not connected.";case Error::NotReady:return "Complete the objective first.";case Error::Overflow:return "The transaction exceeds the supported save bounds.";}return "Unknown action.";}
...
std::size_t Domain::living(TankId id)const{
 const auto fish=std::count_if(state_.fish.begin(),state_.fish.end(),[&](const auto& f){return f.tank==id&&!f.stashed&&!f.dead;});
 return static_cast<std::size_t>(fish);
}
Result Domain::blocker(const Species& s,bool includeCapacity)const{
 ...
 auto* t=tank(state_.activeTank);if(includeCapacity&&(!t||living(t->id)>=static_cast<std::size_t>(t->slots)))return bad(Error::Full);return s.currency==Currency::Gift?Result{}:requireFunds(s.currency==Currency::Coins?quote(s).principal:0,s.currency==Currency::Pearls?s.price:0);
}
...
void Domain::arrangeForSale(std::span<const SellTarget> targets){
 auto assign=[&](FishId id,Motion& motion,bool available){
  std::optional<WorldPoint> next;
  if(available)for(const auto& target:targets)if(target.fish==id&&inTank(target.position))next=target.position;
  if(!next||!motion.sellTarget||next->x!=motion.sellTarget->x||next->y!=motion.sellTarget->y)motion.sellSpeed=0;
  motion.sellTarget=next;
 };
 for(auto& f:state_.fish)assign(f.id,f.motion,!f.egg&&!f.dead&&!f.stashed&&f.tank==state_.activeTank);
}
void Domain::tapWater(WorldPoint point){
 if(!inPlacementWater(point))return;
 if(ripples_.size()>=12)ripples_.erase(ripples_.begin());
 ripples_.push_back({point});
 const auto startle=[&](Fish& f){
  if(f.egg||f.dead||f.stashed||f.tank!=state_.activeTank)return;
  if(std::hypot(f.position.x-point.x,f.position.y-point.y)>260)return;
  f.motion.fleeRemaining=1.1;
 };
 for(auto& f:state_.fish)startle(f);
}
void Domain::stepMovement(double dt,Tool tool,FishId held){
 if(!(dt>0&&dt<=.05))return;
 for(auto& ripple:ripples_){ripple.previousAge=ripple.age;ripple.age+=dt;}
 std::erase_if(ripples_,[](const auto& ripple){return ripple.age>=WaterRipple::lifetime;});
 const double settlingFloor=waterHeight-48;
 for(auto& p:pellets_){p.previous=p.position;p.hasPrevious=true;if(p.position.y<settlingFloor){p.speed=std::min(105.,p.speed+34.*dt);p.position.y=std::min(settlingFloor,p.position.y+p.speed*dt);p.position.x=std::clamp(p.position.x+std::sin(p.phase)*3.*dt,8.,waterWidth-8);p.rotation+=22.*dt;p.phase+=dt;}else{p.position.y=settlingFloor;p.rest+=dt;}}
 ...
 const auto swim=[&](Fish& f){if(f.stashed||f.tank!=state_.activeTank)return;auto& m=f.motion;m.previous=f.position;m.previousPhase=m.phase;m.previousPitch=m.pitch;m.previousFacing=motionFacing(m);m.previousSpeed=m.speed;m.hasPrevious=true;const auto& s=*content_.find(f.species);m.drift+=dt;
  if(tool==Tool::Sell||f.id==held||f.egg||f.dead)m.fleeRemaining=0;
  if(f.egg){const double sink=69.*waterHeight/472.*(.9+.2*(m.cruise-20.)/22.);f.position.y=std::min(settlingFloor,f.position.y+sink*dt);return;}
  if(tool==Tool::Sell){
   // Ease into a nearby resting spot, then stay put. A pressed fish must not
   // move between pointer-down and pointer-up, even during the transition.
   m.foodTarget=0;m.noticeDelay=0;m.dashRemaining=0;m.speed=0;
   double vx=0,vy=0;
   if(m.sellTarget&&f.id!=held&&!f.dead){
    const double dx=m.sellTarget->x-f.position.x,dy=m.sellTarget->y-f.position.y,distance=std::hypot(dx,dy);
    const double response=state_.settings.reducedMotion?12.:8.;
    const double desiredSpeed=std::min(80.,distance*response*.25);
    m.sellSpeed+=(desiredSpeed-m.sellSpeed)*(1-std::exp(-dt*response));
    const double fraction=distance>.05?std::min(1.,m.sellSpeed*dt/distance):1.;
    vx=dx*fraction/dt;vy=dy*fraction/dt;
    f.position.x+=vx*dt;f.position.y+=vy*dt;
    if(std::abs(dx)>2)beginTurn(f,dx>0?1:-1);
    m.speed=std::hypot(vx,vy);
    m.sellSpeed=m.speed;
   }else m.sellSpeed=0;
   if(m.turnRemaining>0){m.turnRemaining=std::max(0.,m.turnRemaining-dt);m.direction=1-m.turnRemaining/m.turnDuration>=.5?-m.turnFrom:m.turnFrom;}
   const double pitch=state_.settings.reducedMotion?0.:std::clamp(std::atan2(vy,std::max(std::abs(vx),20.)),-.28,.28);
   m.pitch+=(pitch-m.pitch)*(1-std::exp(-dt*8.));
   m.phase+=(5.6+std::min(m.speed,80.)*.03)*std::clamp(std::sqrt(46./s.nominalLength),.5,1.45)*dt;
   return;
  }
  m.sellTarget.reset();m.sellSpeed=0;
  Care care=careOf(s,f,state_.simNow);bool hungry=care!=Care::Fed;bool sick=care==Care::Sick;bool heldNow=f.id==held;
  const double halfHeight=s.nominalLength*ageScale(s,f)*.3*tankArtScale;const double top=56.+halfHeight,bottom=waterHeight-halfHeight;
  const bool fleeing=m.fleeRemaining>0;
  if(fleeing){m.dashRemaining=0;m.dashWait=std::max(m.dashWait,1.5);}
  if(!hungry||fleeing){m.foodTarget=0;m.noticeDelay=0;}
  auto target=std::find_if(pellets_.begin(),pellets_.end(),[&](auto& p){return p.id==m.foodTarget;});
  if(target==pellets_.end())m.foodTarget=0;
  if(hungry&&!fleeing&&!heldNow&&!pellets_.empty()&&!m.foodTarget){if(m.noticeDelay<=0)m.noticeDelay=random(.08,.5);m.noticeDelay-=dt;if(m.noticeDelay<=0){auto nearest=std::min_element(pellets_.begin(),pellets_.end(),[&](auto& a,auto& b){return std::hypot(a.position.x-f.position.x,a.position.y-f.position.y)<std::hypot(b.position.x-f.position.x,b.position.y-f.position.y);});m.foodTarget=nearest->id;m.dashRemaining=m.dashDuration;target=nearest;}}
  const bool chase=m.foodTarget&&target!=pellets_.end();m.dashRemaining=std::max(0.,m.dashRemaining-dt);m.dashWait-=dt;if(!fleeing)m.retarget-=dt;m.mealDelay=std::max(0.,m.mealDelay-dt);
  if(!sick&&!chase&&m.dashWait<=0){m.dashRemaining=m.dashDuration;m.dashWait=m.dashInterval*random(.85,1.15);}
  if(m.retarget<=0&&!chase&&!fleeing){m.targetY=random(top,bottom);m.retarget=random(2.5,7.5);}
  ...
  // A tap accelerates the existing swim. Only ordinary edge turns change heading.
  if(fleeing){speed=m.cruise*(state_.settings.reducedMotion?1.3:1.+3.5*std::min(1.,m.fleeRemaining/.65));m.fleeRemaining=std::max(0.,m.fleeRemaining-dt);}
 ...
 for(const auto& arrival:arrivals){
  ...
  if(auto result=execute({.action=Action::Feed,.fish=arrival.fish});result){
   Motion* m=nullptr;if(auto* fish=mutableFish(arrival.fish))m=&fish->motion;
   if(m){m->mealDelay=random(.15,.45);m->retarget=random(.6,1.4);m->foodTarget=0;}pellets_.erase(pellet);
  }
 }

}
void Domain::clearTransient(){
 pellets_.clear();ripples_.clear();arrangeForSale({});
 const auto clear=[](auto& f){f.motion.foodTarget=0;f.motion.noticeDelay=0;f.motion.previous=f.position;f.motion.hasPrevious=false;f.motion.fleeRemaining=0;};
 for(auto& f:state_.fish)clear(f);
}
...
Result Domain::executeImpl(const Command& c){
 ...
 case Action::Buy:{
  ...
  if(state_.fish.size()>=10000||state_.nextFishId==UINT64_MAX)return bad(Error::Maximum);
  const auto offer=quote(*s);if(c.offer&&*c.offer!=offer)return bad(Error::Conflict);
  reason_="egg_purchase";source_=std::to_string(state_.nextFishId);
  ...
  Fish baby=makeFish(s->id,c.point,true);const auto id=baby.id;
  state_.fish.push_back(std::move(baby));
  ...
  count("buy");count("place");emit({"purchase",id,c.point,-coins,0,-pearls,{}});return {.fish=id,.coins=-coins,.pearls=-pearls};
 }
 case Action::Feed:{
 ...
 case Action::Sell:return settle(c);
 case Action::Favorite:{
  if(f){f->favorite=!f->favorite;return {};}
  return bad(Error::InvalidFish);
 }
 case Action::Stash:case Action::Restore:{
  if(!f)return bad(Error::InvalidFish);
  if(c.action==Action::Stash){
   if(f->egg||f->age<4)return {.error=Error::NotReady,.message="Only adult fish can be stored."};
   if(f->stashed||f->tank!=state_.activeTank)return bad(Error::InvalidFish);
   f->stashed=true;f->stashedAt=state_.simNow;return {};
  }
  if(!f->stashed)return bad(Error::NotStored);if(!validPoint(c.point))return bad(Error::InvalidPosition);
  if(living(state_.activeTank)>=static_cast<std::size_t>(tank(state_.activeTank)->slots))return bad(Error::Full);
  f->tank=state_.activeTank;f->position=c.point;f->stashed=false;f->motion.previous=c.point;return {};
 }
 case Action::Move:
  if(!validPoint(c.point))return bad(Error::InvalidPosition);
 ...
 case Action::UnlockTank:case Action::ExpandTank:{
  ...
  const auto* e=nextTankEntitlement(c.tank);if(!e)return bad(Error::Maximum);if(!owned&&level()<e->level)return bad(Error::Level);
 ...
 case Action::SetReducedMotion:state_.settings.reducedMotion=c.value!=0;state_.settings.reducedMotionOverride=true;return {};
 case Action::SetSystemReducedMotion:if(!state_.settings.reducedMotionOverride)state_.settings.reducedMotion=c.value!=0;return {};
 case Action::SetVibration:state_.settings.vibration=c.value!=0;return {};
...
void Domain::fixture(std::string_view name){
 ...
 std::vector<const Species*> available;for(const auto& s:content_.species)if(s.artReady&&s.releaseGate=="Launch")available.push_back(&s);
 ...
 if(name=="inventory"&&!state_.fish.empty()){
  const auto bought=execute({.action=Action::Buy,.key="bubbleEyeGoldfish",.point={544,320}});
  if(bought){auto* f=mutableFish(bought.fish);f->egg=false;f->age=4;f->growthMs=f->purchase.durationMs;execute({.action=Action::Stash,.fish=bought.fish});}events_.clear();
 }
}
} // namespace aq
