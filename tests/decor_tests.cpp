...
Domain atXp(Content c,Amount xp){auto d=ready(std::move(c));auto s=d.state();s.xp=xp;s.highestRewardedLevel=levelFor(d.content(),xp);testing::openingBalances(s);d.install(s);return d;}
...
int main(int argc,char** argv){
 ...
 test("coin and pearl rewards match price and only new item types earn XP",[&]{
  auto d=atXp(content,content.levels[3]);
  for(const auto& [id,expected]:std::vector<std::pair<std::string,Amount>>{{"CP-01",3},{"CD-01",7},{"CP-04",31},{"PP-01",120},{"PD-01",120}}){
   const auto* def=content.findDecor(id);const auto before=d.state();CHECK(def&&def->buyXp==expected);
   CHECK(d.decorPurchaseXp(*def)==expected);const auto result=d.execute(buy(id));CHECK(result&&result.xp==expected);
   CHECK(d.state().xp==before.xp+expected&&d.state().lifetimeXp==before.lifetimeXp+expected);
   CHECK(d.state().wallet.coins==before.wallet.coins-(def->currency==Currency::Coins?def->price:0));
   CHECK(d.state().wallet.pearls==before.wallet.pearls-(def->currency==Currency::Pearls?def->price:0));
   CHECK(d.state().ledger.back().at("reason")=="decor_purchase"&&d.state().ledger.back().at("currency")=="xp");
   CHECK(d.decorPurchaseXp(*def)==0);const auto earned=d.state().lifetimeXp;
   CHECK(d.execute(buy(id)).xp==0&&d.state().lifetimeXp==earned);
  }
 });
 test("purchase retries and reloads cannot repeat the first-copy reward",[&]{
  Domain d(content);auto command=buy("CP-01");command.requestId="plant-reward";
  const auto first=d.execute(command);CHECK(first&&first.xp==3);const auto saved=encode(d.state());
  d.takeEvents();const auto repeated=d.execute(command);CHECK(repeated&&repeated.replayed&&repeated.xp==3);
  CHECK(encode(d.state())==saved&&d.takeEvents().empty());
  d.install(decodeAndValidate(saved,content));CHECK(d.execute(command).replayed);CHECK(encode(d.state())==saved);
  command.requestId="second-plant";CHECK(d.execute(command).xp==0&&d.state().xp==3&&d.state().decor.size()==2);
 });
 test("decor XP crosses a level and saves its grant exactly once",[&]{
  auto d=atXp(content,78);const auto before=d.state();auto command=buy("CP-01");command.requestId="decor-level";
  const auto r=d.execute(command);CHECK(r&&r.xp==3&&r.coins==-25&&d.level()==2&&d.state().xp==81);
  CHECK(d.state().wallet.coins==before.wallet.coins-25+content.levelRewards[1].coins);
  CHECK(d.state().wallet.pearls==before.wallet.pearls+content.levelRewards[1].pearls&&d.state().highestRewardedLevel==2);
  const auto events=d.takeEvents();CHECK(std::count_if(events.begin(),events.end(),[](const auto& e){return e.kind=="level"&&e.reachedLevel==2;})==1);
  const auto saved=encode(d.state());d.install(decodeAndValidate(saved,content));CHECK(d.execute(command).replayed);
  CHECK(encode(d.state())==saved&&d.takeEvents().empty());
 });
 test("failed commits and reward overflow preserve the unused bonus",[&]{
  auto d=atXp(content,78);const auto before=encode(d.state());auto command=buy("CP-01");command.requestId="decor-retry";
  CHECK(d.execute(command,[](const State&){return false;}).error==Error::SaveFailure);
  CHECK(encode(d.state())==before&&d.takeEvents().empty()&&d.decorPurchaseXp(*content.findDecor("CP-01"))==3);
  CHECK(d.execute(command).xp==3&&d.level()==2);
  auto full=atXp(content,0);auto s=full.state();s.lifetimeXp=9'000'000'000'000'000LL;full.install(s);
  const auto overflow=encode(full.state());CHECK(full.execute(buy("CP-01")).error==Error::Overflow);
  CHECK(encode(full.state())==overflow&&full.takeEvents().empty()&&full.decorPurchaseXp(*content.findDecor("CP-01"))==3);
 });
 test("account cap limits visible XP while lifetime XP keeps the full reward",[&]{
  auto d=atXp(content,content.levels.back()-1);const auto before=d.state().lifetimeXp;
  const auto first=d.execute(buy("CP-01"));CHECK(first&&first.xp==1&&d.state().xp==content.levels.back());
  CHECK(d.state().lifetimeXp==before+3);
  auto events=d.takeEvents();CHECK(std::any_of(events.begin(),events.end(),[](const auto& e){return e.kind=="decor"&&e.xp==1;}));
  const auto capped=d.execute(buy("CD-01"));CHECK(capped&&capped.xp==0&&d.state().lifetimeXp==before+10);
  events=d.takeEvents();CHECK(std::any_of(events.begin(),events.end(),[](const auto& e){return e.kind=="decor"&&e.xp==0;}));
  const auto saved=encode(d.state());CHECK(encode(decodeAndValidate(saved,content))==saved);
 });
 test("existing ownership remains eligible for use without retroactive rewards",[&]{
  auto oldContent=content;oldContent.decorations.front().buyXp=0;Domain old(oldContent);
  CHECK(old.execute(buy("CP-01"))&&old.state().xp==0);const auto saved=encode(old.state());
  Domain current(content);current.install(decodeAndValidate(saved,content));CHECK(encode(current.state())==saved);
  CHECK(current.decorPurchaseXp(*content.findDecor("CP-01"))==0);
  CHECK(current.execute(buy("CP-01")).xp==0&&current.state().xp==0&&current.state().decor.size()==2);
 });
 test("fresh and saved games browse and buy decor without a first plant",[&]{
  ...
  CHECK(d.decorPurchaseXp(*content.findDecor("CD-01"))==7);
  CHECK(d.execute(buy("CD-01")));
  CHECK(d.state().wallet.coins==250-content.findDecor("CD-01")->price&&d.state().xp==7);
  CHECK(d.state().decorOnboardingComplete);
  ...
  CHECK(d.state().wallet.coins==250-2*content.findDecor("CD-01")->price&&d.state().xp==7);
  CHECK(d.state().decorOnboardingComplete);
 });
 test("first plant costs 25 and grants 3 XP once",[&]{Domain d(content);const auto before=d.state();CHECK(d.decorPurchaseXp(*content.findDecor("CP-01"))==3);CHECK(d.execute(buy("CP-01")));CHECK(d.state().wallet.coins==225);CHECK(d.state().xp==3);CHECK(d.state().decorOnboardingComplete);CHECK(d.decorScore({1})==10);CHECK(encode(d.state())["fish"]==encode(before)["fish"]);CHECK(d.state().tanks[0].slots==10);CHECK(d.decorPurchaseXp(*content.findDecor("CP-01"))==0);CHECK(d.execute(buy("CP-01",{400,500})));CHECK(d.state().xp==3);CHECK(d.decorScore({1})==10);});
 test("every launch item charges its exact currency and first-ownership XP",[&]{int checked=0;for(const auto& def:content.decorations){auto d=ready(content);const auto before=d.state();const auto r=d.execute(buy(def.id,{544,550}));if(def.level>40){CHECK(r.error==Error::Level);CHECK(encode(before)==encode(d.state()));continue;}++checked;CHECK(r);CHECK(d.state().wallet.coins==before.wallet.coins-(def.currency==Currency::Coins?def.price:0));CHECK(d.state().wallet.pearls==before.wallet.pearls-(def.currency==Currency::Pearls?def.price:0));CHECK(d.state().xp==before.xp&&d.state().lifetimeXp==before.lifetimeXp+def.buyXp);CHECK(d.decorScore({1})==def.score);CHECK(d.execute(buy(def.id,{544,550})));CHECK(d.state().xp==before.xp&&d.state().lifetimeXp==before.lifetimeXp+def.buyXp);CHECK(d.decorScore({1})==def.score);}CHECK(checked>80);});
 ...
 test("64 placed copies is independent of fish slots and stored ownership",[&]{auto d=ready(content);for(int i=0;i<64;++i)CHECK(d.execute(buy("CP-01")));CHECK(d.placedDecor({1})==64);const auto before=encode(d.state());const auto full=d.execute(buy("CP-01"));CHECK(full.error==Error::Maximum);CHECK(full.message=="Store a decoration first. This tank has 64 placed items.");CHECK(encode(d.state())==before);CHECK(d.execute(edit(Action::StoreDecor,1)));CHECK(d.execute(buy("CP-01")));CHECK(d.state().decor.size()==65);CHECK(d.execute(edit(Action::RestoreDecor,1)).error==Error::Maximum);CHECK(d.state().tanks[0].slots==10);});
 ...
 test("decor layering follows base height independently of horizontal perspective",[&]{
  Decoration back{20,"CP-02",{1},{544,350}},front{10,"CD-01",{1},{544,470}};
  CHECK(decorScale(back.position)<decorScale(front.position));CHECK(decorBehind(back,front));CHECK(!decorBehind(front,back));
  // A centred rock sits behind a plant whose base is just one unit lower,
  // even though the rock's perspective scale is larger at the tank centre.
  back.position={535,574};front.position={460,575};
  CHECK(decorScale(back.position)>decorScale(front.position));CHECK(decorBehind(back,front));CHECK(!decorBehind(front,back));
  back.sizeMul=1.7;front.sizeMul=.6;CHECK(decorBehind(back,front));
  for(const double y:{150.,350.,575.})for(const double x:{150.,300.,544.,800.,938.}){
   front.position={x,y};back.position={544,y-1};CHECK(decorBehind(back,front));CHECK(!decorBehind(front,back));
   back.position.y=y+1;CHECK(decorBehind(front,back));
   back.position.y=y;CHECK(decorBehind(front,back));CHECK(!decorBehind(back,front));CHECK(!decorBehind(back,back));
  }
 });
 test("decor footprints remain visible at tank edges with perspective",[&]{
  ...
  CHECK(std::abs(decorScale({544,153.6})-.504)<1e-9);CHECK(std::abs(decorScale({544,504.32})-1.416)<1e-9);
  CHECK(std::abs(decorScale({0,504.32})-1.01472)<1e-9);CHECK(std::abs(decorHaze({544,153.6})-.26)<1e-9);CHECK(decorHaze({544,504.32})<1e-9);
  CHECK(std::abs(decorScale({544,0})-.504)<1e-9);CHECK(std::abs(decorScale({544,635})-1.416)<1e-9);
 ...
 test("paid decor places once with no second charge, gate or XP award",[&]{
  ...
  CHECK(d.state().decor.empty()&&d.state().pendingDecor=="CP-01");CHECK(d.state().wallet.coins==225&&d.state().xp==3);
  ...
  CHECK(d.state().decor.size()==1&&d.state().wallet.coins==225&&d.state().xp==3&&d.state().totalEvents.at("decor")==1);
  ...
  CHECK(d.state().xp==3&&d.state().totalEvents.at("decor")==1);
 });
 test("cancelled Pearl purchases preserve paid copies and first ownership through reload",[&]{
  ...
  CHECK(d.state().wallet.pearls==before.wallet.pearls-def->price&&d.state().xp==before.xp&&d.state().lifetimeXp==before.lifetimeXp+def->buyXp);
  ...
  CHECK(d.state().decor.size()==2&&d.state().xp==before.xp&&d.state().lifetimeXp==before.lifetimeXp+def->buyXp&&d.state().wallet.pearls==before.wallet.pearls-2*def->price);
  ...
  CHECK(d.state().wallet.coins==wallet.coins&&d.state().wallet.pearls==wallet.pearls&&d.state().xp==before.xp&&d.state().lifetimeXp==before.lifetimeXp+def->buyXp);
 });
 test("pending copies reserve inventory and can be cancelled when the tank fills",[&]{
  auto d=ready(content);for(int i=0;i<64;++i)CHECK(d.execute(buy("CP-01")));
  CHECK(d.execute(edit(Action::StoreDecor,1)));CHECK(d.execute({Action::PurchaseDecor,{}, {},"CP-01"}));
  CHECK(d.execute(edit(Action::RestoreDecor,1)));CHECK(d.placedDecor({1})==64);
  ...
  CHECK(d.execute({Action::CancelDecor}));CHECK(d.state().decor.size()==65&&d.decoration(65)->stored&&d.placedDecor({1})==64);
