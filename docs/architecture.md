...
`Content` holds schema 4 configuration from the v4 workbook. `Domain` owns the game state, care clocks, movement, free food and accepted commands. `src/economy.cpp` owns integer quote calculations, request receipts, the ledger and terminal sale settlements. `src/decor.cpp` owns decorative placement and ownership. These modules have no SDL or filesystem dependency.

Every fish carries a purchase snapshot. Its future reward does not depend on the current catalog or account level. Coin and pearl purchases create the same fish type and use the same growth, feeding, movement and sale code. Pearl purchases have zero refundable coin principal. Care advances across owned tanks; only the active tank moves visually. The movement step is 20 milliseconds, while growth uses exact millisecond intervals.
...
`Storage` serializes explicit version 4 fields, validates decoded candidates, reconciles balances and uses temporary-file replacement with backups. It converts earlier v4 adult-only fish records into the single fish collection. Versions before v4 remain unsupported. Offline growth consumes only the remaining meal duration and is applied once per saved wall-time interval.
